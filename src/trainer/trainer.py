...
class Trainer:
    ...
    def train(self, train, val, epochs=1, lr=1e-4, restarts=10, ckpt_dir='checkpoints', batch_size=32):
        ...
        ## from scratch
        ckpts = os.listdir(self.wdir/ckpt_dir)
        if not any(ckpts):
        ...
        ## resume
            ...
            latest = sorted([ckpt.split('_')[-1] for ckpt in ckpts], key=lambda x: int(x.split('_')[-1]))[-1]
            self.load(ckpt_dir, latest)
            self.optim = self.optim_base(self.model.parameters(), lr=lr)
            self.scheduler = self.scheduler_base(self.optim, restarts)
            strt = int(latest)+1
            print(f"Picking up training at epoch {strt}")
        ...
        for epoch in range(strt, strt+epochs):
            ...
            with torch.enable_grad():
                ...
                for i, batch in enumerate(ProgBar(train_loader, desc=f'Training Batches')):
                    ...
                    loss, logger = self.model.loss(batch, self.loss, logger)
            ...
            with torch.no_grad():
                ...
                for batch in ProgBar(val_loader, desc=f'Validating Batches'):
                    loss, logger = self.model.loss(batch, self.loss, logger, val=True) ## add validation flag because the adversarial loss breaks the logic, so its not evaluated.
