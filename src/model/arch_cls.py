...
class H0_mini_for_Adversarial_on_CLS(nn.Module):
    ...
    def loss(self, batch, loss, logger=None, val=False):  
        ...
        s_recon = self.entangler.probas_to_specified(batch['labels'].to(self.device))
        ...
        return loss(batch, s_classif, z_classif, rec_img, s_recon, s, self.device, logger, val, None, None)
...
class Entangler(nn.Module):
    def __init__(self, n_features, n_stain_features, n_other_features, n_classes):
        ...
        ### disentanglement
        self.to_specified = LinearProjector(n_features, n_stain_features)
        self.to_unspecified = LinearProjector(n_features, n_other_features)
        ...
        ### reentanglement
        self.probas_to_specified = nn.Linear(n_classes, n_stain_features)
        self.act2 = nn.ReLU()
        self.reentangler = nn.Linear(n_stain_features+n_other_features, n_features)
    
    def disentangle(self, tokens):
        patches = tokens[:,0,:] ## extract cls tokens
        z = self.to_unspecified(patches)
        s = self.to_specified(patches)
        return s, z # -> [B, n_stain_features], [B, n_other_features]
    
    def reentangle(self, s, z):
        patches = torch.cat([s, z], dim=1)
        patches = self.reentangler(patches)
        return patches.unsqueeze(-1).unsqueeze(-1)
