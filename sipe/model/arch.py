# changed import to pep8 style, first block is stdlib, second block is third party, third block is local imports
import datetime
import os
...
TOKEN_PATH = Path("token.txt").resolve()


def get_encoder_and_transforms(base_model):

    if TOKEN_PATH.exists(): # you should not need this, but use dotenv for that kind of stuff
...
class H0_mini_for_Adversarial(nn.Module):
    def __init__(
        self,
        possible_classes,
        base_model="hf-hub:bioptimus/H0-mini",
        emb_stain_size=64,
        device="cuda:0",
    ):
        super().__init__()
        ...
        self.possible_classes = sorted(
            self.defrag(list(possible_classes.keys()))
        )  ## for reproducibility
        self.enc = MultiLabelBinarizer()
        self.enc.fit(self.possible_classes)
        self.n_classes = len(self.enc.classes_)

        self.backbone, self.transform = get_encoder_and_transforms(
            base_model
        )  ## backbone model
        print("Backbone built!")

        self.entangler = Entangler(
            768, emb_stain_size, 768 - emb_stain_size, self.n_classes
        )
        print("Disentangler built!")

        self.image_decoder = get_decoder(
            num_features, 3
        )  ## decoder for image recon, adds a projection layer to mix stain
    ...
    def forward(self, batch):
        if type(batch) == dict:
            ...
            tokens = self.backbone(batch.to(self.device))  # [B, 261, 768]
    ...
    def loss(self, batch, loss, logger=None, val=False):
        ...
        batch["labels"] = torch.tensor(
            self.transform_labels([s["staining"] for s in batch["metadata"]]),
            dtype=torch.float32,
        )
        ...
        if type(batch) == dict:
            disc_gt = self.discriminator(batch["image"].to(self.device))
        else:
            disc_gt = self.discriminator(batch.to(self.device))
    ...
    def load(self, pth):
        ...
        try:
            self.discriminator.load_state_dict(torch.load(pth / "discriminator.pth"))
        except:
            print("Couldnt load discriminator, using initialized weights instead")
    ...
    def defrag(self, raw_labels):
        ...
        for label in raw_labels:
            ...
            ## hematox & eosin
            if (
                label == "HE - Hematoxylin and eosin stain method (procedure)"
                or label == "Hematoxylin and eosin stain method"
                or label == "hematoxylin stain+water soluble eosin stain"
            ):
                new_labels.append("Hematoxylin+Eosin")
            ## Van Gieson
            elif (
                label == "Van Gieson stain"
                or label == "Verhoeff-Van Gieson stain method"
            ):
                ...
                "Herovici's stain method" == label or "Herovic's stain method" == label
            ):
                new_labels.append("Herovicis stain method")
            else:
        ...
        for lbl in new_labels:
            ...
            if len(sep) > 1:
                ...
                for l in sep:
                    l = l.strip().lower()
                    if "hematoxylin" in l:
                        new_sep.append("hematoxylin")
                    elif "eosin" in l:
                        new_sep.append("eosin")
                    elif "s-100" in l:
                        new_sep.append("s100")
                    else:
                        new_sep.append(l)
